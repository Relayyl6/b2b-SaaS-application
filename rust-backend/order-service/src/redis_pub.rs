...
impl RedisPublisher {
    ...
    /// Publishes an event with a strict timeout budget.
    pub async fn publish<T: serde::Serialize>(
        ...
        self.publish_payload(channel, payload).await
    }
    ...
    /// Publishes in a detached task so HTTP response paths do not block.
...
mod tests {
    ...
    async fn noop_publish_returns_ok() {
        ...
        let result = publisher
            .publish("order.test", &serde_json::json!({"ok": true}))
            .await;
        assert!(result.is_ok());
    }
}
